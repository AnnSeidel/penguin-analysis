#!/bin/bash
#SBATCH -J "sludgeMetaT-rnaspades"
#SBATCH -p hh
...
OUTDIR_BASE="../benchmark/rnaspades"
OUTDIR="${OUTDIR_BASE}/${SAMPLE}_1P_2P"
mkdir -p ${OUTDIR}


if [[ ! -f ${OUTDIR}/hard_filtered_transcripts.fasta ]];then
	...
	# command call
	RNASPADES_COMMAND="/cbscratch/annika/virus-assembly-project/software/benchmark_versions/SPAdes-3.15.2-Linux/bin/rnaspades.py \
            -1 ${READSFILE1} -2 ${READSFILE2} -o ${TMP}/assembly \
            --tmp-dir "${TMP}" -t ${SLURM_CPUS_ON_NODE}"
    /usr/bin/time -f "%e,%M" --output ${TMP}/${SAMPLE}.rnaspades.time.mem.log ${RNASPADES_COMMAND} > ${TMP}/${SAMPLE}.rnaspades.log
	
	# copy result back and rename
	cp ${TMP}/*log ${OUTDIR}/
	cp -r ${TMP}/assembly/* ${OUTDIR}/
	sed -i "s/^>/>${SAMPLE}_/g" ${OUTDIR}/hard_filtered_transcripts.fasta
...
# filter fasta file by length
  bioawk -c fastx -v LEN=${LENCUTOFF} 'length($seq)>=LEN{print ">"$name" "$comment"\n"$seq}' ${OUTDIR}/hard_filtered_transcripts.fasta > ${OUTDIR}/hard_filtered_transcripts-${LENCUTOFF}.fasta
  if [[ ! -f ${OUTDIR}/detection_${SAMPLE}/contigs.ofInterest.fa ]]; then
    ./../evaluation/getContigsOfInterest.sh ${OUTDIR}/hard_filtered_transcripts-${LENCUTOFF}.fasta ${OUTDIR_BASE}/detection_${SAMPLE}
  fi
