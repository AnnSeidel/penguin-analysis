#!/bin/bash
#SBATCH -J "sludgeMetaT-metaspades"
#SBATCH -p hh
...
OUTDIR_BASE="../benchmark/metaspades"
...
if [[ ! -f ${OUTDIR}/scaffolds.fasta ]]; then
	...
	# command call
	METASPADES_COMMAND="/cbscratch/annika/virus-assembly-project/software/benchmark_versions/SPAdes-3.15.2-Linux/bin/metaspades.py \
            -1 ${READSFILE1} -2 ${READSFILE2} -o ${TMP}/assembly \
            --tmp-dir "${TMP}" -t ${SLURM_CPUS_ON_NODE}"
	/usr/bin/time -f "%e,%M" --output ${TMP}/${SAMPLE}.metaspades.time.mem.log ${METASPADES_COMMAND} > ${TMP}/${SAMPLE}.metaspades.log
